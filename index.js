...
function loadGame(){
const s = localStorage.getItem("game");
...
document.getElementById("payPercent").textContent =
`Pay Percent: ${gameState.payPercent}`;

document.getElementById("BitcoinDisplay").textContent =
`1 BTC: ${gameState.BitcoinVal}`;

document.getElementById("Bitcoins").textContent =
`BTC: ${gameState.Bitcoin}`;

// items
for(const key in prices){
const el = document.getElementById(key.replace("Count","") + "Display");
if(el){
el.textContent = key.replace("Count","") + ": " + gameState[key];
}
}

// stock
for(const k in stock){
const el = document.getElementById(k + "Stock");
if(el) el.textContent = "Stock: " + stock[k];
}
...
const sell = document.getElementById("sell-" + id);
const sellAll = document.getElementById("sell-all-" + id);
...
// ---------------- CRYPTO ----------------
function investBitcoin(){
let amt = +cryptoInput.value;
if(amt <= 0 || gameState.cashCount < amt) return;

gameState.cashCount -= amt;
gameState.Bitcoin += amt / gameState.BitcoinVal;
...
// price movement
setInterval(()=>{
gameState.BitcoinVal += Math.floor(Math.random()*2000-1000);
if(gameState.BitcoinVal < 1000) gameState.BitcoinVal = 1000;
updateUI();
},2000);
